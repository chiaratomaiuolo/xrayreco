...
reconstruction strategy based on nearest-neighbors.
"""
...
from xrayreco.preprocessing import Xraydata, processing_data, recon_data, highest_pixel_coordinates
...
if __name__ == "__main__":
    ...
    #recon_test_data = ReconInputFile(XRAYRECO_DATA / 'hxsim_20ENC_01srcsigma_test_recon.h5')

    # Extracting recon columns
    ...
    #Preprocessing test data for obtaining input and target
    test_input_data, test_target_data = processing_data(test_data)
    # Loading scaler and applying it on test data
    scaler = joblib.load(XRAYRECO_TRAINING / 'scaler.gz')
    X = scaler.fit_transform(test_input_data.reshape(-1, test_input_data.shape[-1])).reshape(test_input_data.shape)


    # Compiling the models and loading the latest weights
